...
def test_should_method_shouldIgnore_return_False_when_hole_is_not_open(mocker):
    ...
    task = UseShovelTask(waypoint)
    isHoleOpenSpy = mocker.patch('src.repositories.gameWindow.core.isHoleOpen', return_value=False)
    assert task.shouldIgnore(context) == False
    isHoleOpenSpy.assert_called_once_with(context['gameWindow']['image'], gameWindowCore.images[context['resolution']]['holeOpen'], context['radar']['coordinate'], waypoint['coordinate'])
...
def test_should_method_shouldIgnore_return_False_when_hole_is_open(mocker):
    ...
    task = UseShovelTask(waypoint)
    isHoleOpenSpy = mocker.patch('src.repositories.gameWindow.core.isHoleOpen', return_value=True)
...
def test_should_do(mocker):
    ...
    clickSlotSpy = mocker.patch('src.repositories.gameWindow.slot.clickSlot', return_value=True)
    ...
    pressSpy.assert_called_once_with(context['hotkeys']['shovel'])
